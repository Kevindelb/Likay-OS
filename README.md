...
Este proyecto parte de kal, que **no es un agente de IA — es un kernel agéntico**: la infraestructura de seguridad (permisos explícitos de filesystem/red, sandboxing con Docker, auditoría verificable, protección activa contra presión de RAM) sobre la que hoy corre un agente de referencia, que orquesta modelos de IA (locales vía Ollama, o en la nube) para resolver tareas. Corre hoy como una aplicación sobre Windows/Linux. Likay-OS lleva esa misma base de seguridad un paso más allá: en vez de ser una app que el usuario abre, es el sistema que arranca directo y ya está listo para trabajar.
